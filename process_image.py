...
def AI_inputs(img,x_bird=None,y_center_bird=None,x_end_pipe=None , y_center_hole=None):
    ...
    if max_val>restart_buttom["trashold"]:
    ...
    if max_val>bird["trashold"]:
        update_frame(frame,max_loc,bird["img"])
        x_bird= max_loc[0]
    ...
    yloc,xloc = np.where(result>=pipe_up["trashold"])
    ...
    if len(xloc)>0:
        ...
        if len(nexts_pipe_y)>0:
            update_frame(frame,(nexts_pipe_x[0],nexts_pipe_y[0]),pipe_up["img"])
    ...
    yloc,xloc = np.where(result>=pipe_down["trashold"])
    ...
    if len(xloc)>0:
        ...
        nexts_pipe_y = nexts_pipe_y[nexts_pipe_xinds[::-1]]
        if len(nexts_pipe_y)>0:
            update_frame(frame,(nexts_pipe_x[0],nexts_pipe_y[0]),pipe_down["img"])
